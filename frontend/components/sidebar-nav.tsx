...
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    section: 'Data',
    items: [
      { href: '/dashboard', label: 'Data Sources', icon: dataSourcesIcon },
    ],
  },
  {
    section: 'Protect',
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 md:hidden rounded-md bg-white p-2 shadow-md border border-gray-200"
      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-5 border-b border-gray-100 shrink-0">
          <div className="min-w-0">
            <h1 className="text-base font-semibold text-gray-900 tracking-tight">QueryAble</h1>
            <p className="text-[11px] leading-tight text-gray-400">De-identify & query clinical data</p>
          </div>
          <button
            onClick={() => setMobileOpen(false)}
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {NAV_SECTIONS.map((section, sIdx) => (
            <div key={section.section ?? `group-${sIdx}`} className={sIdx > 0 ? 'pt-3' : ''}>
              {section.section && (
