...
class PDFParser:
    ...
    def extract_text(self, file_path: str) -> ParsedPDF:
        """Main entry point. Returns a ParsedPDF with raw text and metadata."""
        path = Path(file_path)
        if not path.exists():
            return ParsedPDF(
    ...
    def _extract_with_pdfplumber(self, file_path: str) -> ParsedPDF:
    ...
    def _extract_with_ocr(self, file_path: str, page_count: int) -> ParsedPDF:
        try:
